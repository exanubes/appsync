...
func main() {
	var (
		...
		authToken    = requiredEnv("APPSYNC_AUTH_TOKEN") // Cognito ID token, OIDC token, or Lambda authorizer token
	...
	payload, err := json.Marshal(event{Message: "hello from token example"})
	if err != nil {
		log.Fatal(err)
	}
